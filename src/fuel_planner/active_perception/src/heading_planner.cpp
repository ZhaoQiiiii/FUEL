    ...
    for (int i = 0; i < yaws.size(); ++i) {
        ...
        if (start_end) { // start and end vertice
            ...
            for (int j = 0; j < vert_num; ++j) { // evaluate info gain in parallel
                ...
                futs[j] =
                    std::async(std::launch::async, &HeadingPlanner::calcInformationGain, this, pts[i], ys, ctrl_pts, j);
            }
    ...
    for (int x = lbi[0]; x <= ubi[0]; ++x)
        for (int y = lbi[1]; y <= ubi[1]; ++y)
            for (int z = lbi[2]; z <= ubi[2]; ++z) {
                // subsampling
                if (!(x % factor == 0 && y % factor == 0 && z % factor == 0))
                    continue;
                // check visibility of unknown cells in FOV, 1: accessible, 2: blocked
                pt_idx << x, y, z;
                if (!sdf_map_->getOccupancy(pt_idx) == SDFMap::UNKNOWN)
                    continue;
                if (!sdf_map_->isInBox(pt_idx))
                    continue;
                sdf_map_->indexToPos(pt_idx, check_pt);
                if (!insideFoV(check_pt, pt, normals))
                    continue;
...
void HeadingPlanner::calcFovAABB(const Eigen::Matrix3d &R_wc, const Eigen::Vector3d &t_wc, Eigen::Vector3i &lb,
                                 Eigen::Vector3i &ub) {
