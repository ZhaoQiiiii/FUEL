...
void VisibilityUtil::calcViewConstraint(NonUniformBspline &traj, ViewConstraint &cons) {
    ...
    if (!findUnknownPoint(traj, unknown, unknown_t))
        return;

    if (!findCriticalPoint(traj, unknown, unknown_t, critic, crit_t))
        return;

    if (!findDirAndIdx(traj, unknown_t, crit_t, dir, idx, min_pt))
        return;
