...
template <typename NodeT> class GraphSearch {
  public:
    GraphSearch() {
        node_num_ = 0;
        edge_num_ = 0;
...
template <typename NodeT> class NodeCompare {
  public:
...
template <typename NodeT> void GraphSearch<NodeT>::print() {
    for (auto v : nodes_) {
        v->print();
        v->printNeighbors();
    }
...
template <typename NodeT> void GraphSearch<NodeT>::addNode(const shared_ptr<NodeT> &node) {
    nodes_.push_back(node);
    nodes_.back()->id_ = node_num_++;
}

template <typename NodeT> void GraphSearch<NodeT>::addEdge(const int &from, const int &to) {
